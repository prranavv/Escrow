...
fn initialize_and_refund(){
    ...
    let refund_descriminator_idl=[
      ...
      let mut refund_descriminator = [0u8; 8];
      refund_descriminator.copy_from_slice(&refund_descriminator_idl[..8]);

      let refund_instruction_data = refund_descriminator.to_vec();
      let refund_instruction=Instruction{
        ...
        data:refund_instruction_data
      };

    let refund_tx = Transaction::new_signed_with_payer(&[refund_instruction], Some(&maker.pubkey()), &[&maker], svm.latest_blockhash());
    svm.send_transaction(refund_tx).unwrap();

    let maker_mint_a_ata_state =litesvm_token::get_spl_account::<spl_token::state::Account>(&svm, &maker_mint_a_ata).unwrap();
    assert_eq!(maker_mint_a_ata_state.amount,1_000_000_000,"Maker should have deposited the amount to the vault");
    
    let vault_state = match svm.get_account(&vault){
        None=>true,
        Some(account)=>account.lamports==0 && account.data.is_empty()
    };
    assert!(vault_state,"Vault is closed")
}